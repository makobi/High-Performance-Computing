...
int stepNbodyModelLeapfrog(NbodyModel * theModel, double tStep) {
    int i, count, start, stop;

#ifdef HAS_MPI
    extern int size;
    extern int rank;

    count = theModel->n*6 / size;
    start = rank * count;
    stop = start + count;
#endif
    ...
    if(theModel->t<0.5*tStep) {
        ...
        // update v,x
#ifdef HAS_MPI
        double *buffer = (double *)calloc(theModel->n*6 * sizeof(double));
...
//        for (i=0; i<theModel->n*6;i++) {
        ...
        MPI_Allreduce(buffer, theModel->X, theModel->n*6, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
	free(buffer);
